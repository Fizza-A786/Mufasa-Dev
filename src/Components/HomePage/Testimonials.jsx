      ...
      <div className="max-w-[1280px] mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {testimonials.map((testimonial) => (
            <div key={testimonial.id} className="reveal p-8 bg-[#0A0A0A] rounded-xl border border-white/10">
              <div className="flex gap-1 mb-4">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className="text-[#F46F25] fill-[#F46F25]" size={20} />
                ))}
              </div>
              <p className="text-[#999999] mb-6 italic">"{testimonial.quote}"</p>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold" style={{ fontFamily: "Barlow Condensed, sans-serif" }}>{testimonial.name}</h4>
                  <p className="text-[#999999] text-sm">{testimonial.role}</p>
                </div>
              </div>
            </div>
          ))}
